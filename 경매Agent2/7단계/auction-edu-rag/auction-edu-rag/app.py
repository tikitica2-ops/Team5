...
# Streamlit Cloud의 Secrets를 환경변수로 브리지한다(로컬은 .env 사용).
# providers의 OpenAI/Anthropic SDK가 환경변수에서 키를 읽기 때문.
try:
...
@st.cache_resource(show_spinner="지식베이스 준비 중...")
def ensure_db():
    """chroma_db가 없으면 최초 1회 구축(배포 환경 대비). 50청크라 빠름."""
    import ingest
    if not (os.path.exists(config.CHROMA_DIR) and os.listdir(config.CHROMA_DIR)):
...
try:
    ...
    st.error(
        f"지식베이스 구축에 실패했습니다: {e}\n\n"
        "Secrets(또는 .env)에 OPENAI_API_KEY가 설정됐는지 확인하세요."
    )
